...
※ Modern Toolkit の type 名は Classic と重なるものが多く(auto / card / center / fit / form / hbox / vbox)、本ライブラリのレイアウトレジストリは type 名ベースなのでこれらは同名の実装でカバーされる。`carousel` / `float` は未対応。grid は similar-extjs 独自。
