...
### AI アシスト (Playground 内で DSL を AI に修正させる)

エディタ下部の「🤖 AI アシスト」を開き、⚙ から接続先を設定すると、チャットで修正指示が出せる
(例:「受注一覧に単価列を追加して」)。修正案は**差分プレビュー**で確認してから適用する。

- **プロバイダ**: OpenAI 互換 (OpenAI / Ollama / LM Studio / vLLM など) または Anthropic 互換。
  BASE-URL / API キー / モデル名を設定 (LocalStorage にのみ保存。API キーは設定した
  エンドポイント以外には送信されない)
- **ステートレス方式**: 毎回のリクエストに「現在のエディタ内容 + 指示」を添付するので、
  手動編集と AI 修正を交互に行ってもズレない (会話履歴は表示用)
- system プロンプトには使用コンポーネントのリファレンスが自動で入る。応答の DSL は
  構文検証され、エラー時は 1 回だけ自動リトライする
- ローカル LLM (Ollama) を使う場合は CORS 許可が必要: `OLLAMA_ORIGINS='*' ollama serve`
...
- **配布用** `skills/uivolve-mock/` — uivolve を使っていないプロジェクトでも動く自己完結版。
  書き方規約 (`references/dsl-guide.md`)・xtype リファレンス・JSON Schema に加えて、
  **サンプル集** (`references/samples/` — Playground の全 26 画面 + MDX 仕様書フル例 2 本を
  カテゴリ別一覧と xtype / layout 逆引きの INDEX.md 付きで収録) を同梱し、
...
`skills/uivolve-mock/references/` の components.md / dsl.schema.json / samples/ は
`npm run docs` / `npm run schema` (scripts/sync-skill-refs.mjs) が
Playground のサンプルと MDX デモから自動生成・同期するので直接編集しない。
